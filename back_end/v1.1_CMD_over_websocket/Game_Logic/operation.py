    ...
    for cur_asset in gamer.properties:
        if cur_asset.block_id == asset_number:
            if cur_asset.house_num == 6:
                ...
                push2all("%s built one hotel in %s" % (gamer.name, cur_asset.name))
            else:
...
def push2all(line):
    server_input = line
    push_service.game_log.append(("NPC", line))
    for key in push_service.clients.keys():
        push_service.clients[key]["object"].write_message(server_input)
        print("Write to client %s: %s" % (key, server_input))

def wait_choice():
    choice = push_service.global_Choice.get_choice()
    while choice == -1:
        choice = push_service.global_Choice.get_choice()
    return choice
