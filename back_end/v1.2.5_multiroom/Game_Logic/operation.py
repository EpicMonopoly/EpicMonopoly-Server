...
def push2all(line):
    roomid = game_entrance.get_roomid()
    conn = game_entrance.get_conn()
    return push_service.push2all_r(line, roomid, conn)


def wait_choice():
    roomid = game_entrance.get_roomid()
    conn = game_entrance.get_conn()
    return push_service.wait_choice_r(roomid, conn)
